...
def read_records(table_name, erp, company_short):
    logger.debug(
...
def select_db_table(conn, df, table_name):
    ...
    elif (table_name == "dw_test_value_entries"):
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            cursor.execute(insert_string, row.entry_no, row.entry_type, row.item_ledger_entry_type, row.document_type, row.document_no, row.item_no, row.global_dimension_1_code, row.global_dimension_2_code, row.location_code, row.user_id, row.item_ledger_entry_no, row.gen_bus_posting_group, row.gen_prod_posting_group,
                           row.document_date, row.posting_date, row.cost_amount_actual, row.sales_amount_actual, row.invoiced_quantity, row.item_ledger_entry_quantity, row.salespers_purch_code, row.system_created_by, row.dw_evi_bu, row.dw_erp_system, row.dw_erp_source_table, row.dw_timestamp, row.dw_value_entry_id)
    elif (table_name == "dw_test_resource_ledger_entries"):
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            cursor.execute(insert_string, row.entry_no, row.posting_date, row.entry_type, row.document_no, row.resource_no, row.description, row.global_dimension_1_code, row.global_dimension_2_code, row.quantity, row.unit_cost, row.total_cost, row.unit_price, row.total_price, row.user_id,
                           row.shortcut_dimension_3_code, row.shortcut_dimension_4_code, row.shortcut_dimension_5_code, row.shortcut_dimension_6_code, row.shortcut_dimension_7_code, row.shortcut_dimension_8_code, row.dw_evi_bu, row.dw_erp_system, row.dw_timestamp, row.dw_erp_source_table, row.dw_resource_ledger_entry_id)
    elif (table_name == "dw_test_posted_sales_invoices_headers"):
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            cursor.execute(insert_string, row.order_no, row.sell_to_customer_no, row.amount, row.bill_to_customer_no, row.posting_date, row.salesperson_code, row.shortcut_dimension_1_code, row.shortcut_dimension_2_code, row.location_code, row.document_date, row.external_document_no, row.payment_terms_code,
                           row.shipment_method_code, row.ssi_closed_date, row.system_created_by_user, row.system_created_by, row.system_created_at, row.system_modified_at, row.fieldpoint_workorder, row.dw_evi_bu, row.dw_erp_system, row.dw_timestamp, row.dw_erp_source_table, row.dw_posted_sale_invoice_header_id)
    elif (table_name == "dw_test_posted_sales_invoices_lines"):
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            cursor.execute(insert_string, row.document_no, row.line_no, row.sell_to_customer_no, row.type, row.no, row.description, row.shortcut_dimension_1_code, row.shortcut_dimension_2_code,
                           row.quantity, row.unit_price, row.amount, row.amount_including_vat, row.dw_evi_bu, row.dw_erp_system, row.dw_erp_source_table, row.dw_timestamp, row.dw_posted_sale_invoice_line_id)
    elif (table_name == "PostedSalesCreditMemoHeaders"):
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            cursor.execute(insert_string, row.no, row.sell_to_customer_no, row.due_date, row.amount, row.amount_including_vat, row.sell_to_contact, row.bill_to_customer_no, row.posting_date, row.salesperson_code, row.shortcut_dimension_1_code, row.shortcut_dimension_2_code, row.location_code,
                           row.document_date, row.system_created_by_user, row.system_created_at, row.system_created_by, row.system_modified_at, row.system_modified_by, row.dw_evi_bu, row.dw_erp_system, row.dw_erp_source_table, row.dw_timestamp, row.dw_posted_sale_credit_memo_header_id)
    elif (table_name == "PostedSalesCreditMemoLines"):
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            cursor.execute(insert_string, row.document_no, row.line_no, row.sell_to_customer_no, row.type, row.no, row.description, row.shortcut_dimension_1_code, row.shortcut_dimension_2_code,
                           row.quantity, row.unit_price, row.amount, row.amount_including_vat, row.dw_evi_bu, row.dw_erp_system, row.dw_timestamp, row.dw_erp_source_table, row.dw_posted_sale_credit_memo_line_id)
...
def read_csv_and_insert_to_dw_2(company, endpoint, date_time_string, sql_table, df_sqlserver, column_name):
    ...
    df = pd.read_csv(
        f'C:/Users/eviadmin/Documents/Datawarehouse/python_scripts/DataLake/test/files/from_datalake/{company}_{endpoint}_{date_time_string}.csv.gz',
        compression="gzip")
    df['entry_no'] = df['entry_no'].astype(str)
    ...
    if (len(sys.argv) == 3):
        ...
        if ((endpoint == "ItemLedgerEntries") | (endpoint == "ValueEntries") | (endpoint == "ResourceLedgerEntries")):
            ...
            df_sqlserver = read_records(
                data_to_read[endpoint], "BC", company_short)
